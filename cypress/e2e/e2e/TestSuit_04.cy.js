...
describe('smoke test suite', function (done) {
    ...
    it('Test case to verify Findings Summary api ', () => {
        var FindingsSummary_req = {
            "TPPId": TPPId
        }
        cy.wait(2000);
        cy.log('Hover on help text')
        cy.xpath('//div/img[@class="focus0"]').click({
            force: true
        })
        cy.wait(2000);
        cy.xpath('//h4[@class="font_weight_bold my-4"]').invoke('text').then((Finding) => {
            cy.log("Verify Finding Summary Header: " + Finding);
            expect('Findings Summary').equal(Finding)
        })
        cy.xpath('//h4[@class="font_weight_bold my-4"]').click({
            force: true
        })
        cy.wait(2000);
        cy.xpath('//button[@class="btn btn-lg btn-brown rounded_25 fs-6 float-end" and @id="focus0" ]').invoke('text').then((Impact) => {
            cy.log("Verify Impact button    : " + Impact);
            expect('Impact ').equal(Impact)
        })

        cy.xpath('//button[@class="btn btn-lg btn-brown rounded_25 fs-6 float-end" and @id="focus0" ]').click({
            force: true
        })
        cy.wait(2000);
        cy.xpath('//h4[@class="font_weight_bold my-4"]').click({
        ...
        cy.request({
            url: "https://dev.k4coach.com/backend/findingsSummary",
            method: "POST",
            auth: Auth,
            ...
            cy.readFile('cypress/fixtures/success.json').then(obj => {
                var abc = obj;
                expect(abc.focus[0].measure.measureValue).equal(response.body.focus[0].measure.measureValue)
                expect(abc.focus[0].finding.findingValue).equal(response.body.focus[0].finding.findingValue)
                cy.get('.flex_grow_measurename > .font_weight_bold').invoke('text').then((invk) => {
                    cy.log("This is Cognitive text: " + invk);
                    expect(abc.focus[0].measure.measureName).equal(invk)
                })
                expect(abc.focus[0].sourceMeasure[0].measure.measureValue).equal(response.body.focus[0].sourceMeasure[0].measure.measureValue)
                expect(abc.focus[0].sourceMeasure[0].finding.findingValue).equal(response.body.focus[0].sourceMeasure[0].finding.findingValue)
                cy.get(':nth-child(2) > .findings_container > .flex_grow_measurename > .pl_60 > .font_weight_bold').invoke('text').then((invk) => {
                    cy.log("This is Growth Behaviour: " + invk);
                    expect(abc.focus[0].sourceMeasure[0].measure.measureName).equal(invk)
                })

                expect(abc.focus[0].sourceMeasure[1].measure.measureValue).equal(response.body.focus[0].sourceMeasure[1].measure.measureValue)
                expect(abc.focus[0].sourceMeasure[1].finding.findingValue).equal(response.body.focus[0].sourceMeasure[1].finding.findingValue)
                cy.get(':nth-child(3) > .findings_container > .flex_grow_measurename > .pl_60 > .font_weight_bold').invoke('text').then((invk) => {
                    cy.log("This is Growth Context: " + invk);                   
                    expect(abc.focus[0].sourceMeasure[1].measure.measureName).equal(invk)
                })
        ...
        cy.xpath('//*[@id="main_body_div"]/div[2]/div/div[9]/div/div/div/div/div[2]/h5/button').scrollIntoView({
            duration: 5000
        })
        cy.xpath('//*[@id="main_body_div"]/div[2]/div/div[9]/div/div/div/div/div[2]/h5/button').invoke('text').then((view_Insight) => {
            cy.log("clicking on the button : " + view_Insight);

            expect(view_Insight).equal(view_Insight)
        })

        cy.xpath('//*[@id="main_body_div"]/div[2]/div/div[9]/div/div/div/div/div[2]/h5/button').click({
            force: true
        })
        cy.wait(3000)
    })
    it('Test case to verify Terms of Use & Privacy policy', () => {
        cy.xpath('//*[@id="app"]/div/div[4]/div//p/span[2]').invoke('text').then((Terms_of_Use) => {
